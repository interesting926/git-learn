/*这里的QR_Encode.c和压缩包里的不一样,是修改过的,主要区别是把很多固定内容的数组加了code 把它们放入了rom
节省ram的空间 不然STC89C516RD+空间不够的 空间不够的现象就是 LCD啥也出不来*/
...
const QR_VERSIONINFO code QR_VersonInfo[] = {{0}, // Ver.0
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
int m_ncDataBlock;
unsigned char  data	 m_byBlockMode[MAX_DATACODEWORD];
unsigned char  data	 m_nBlockLength[MAX_DATACODEWORD];
